...
create policy if not exists "Anyone can create inquiry"
  on public.inquiries
  for insert
...
create policy if not exists "Bookings are viewable by everyone"
  on public.bookings
  for select
  using (true);

-- Only authenticated users can create/update/delete bookings (optional for future admin UI)
create policy if not exists "Authenticated can manage bookings"
  on public.bookings
  for all
  to authenticated
  using (true)
